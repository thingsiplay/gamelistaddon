...
def dict_to_element(tag, d, source=None):
...
# Shortcut
def dict_to_game_element(d, source=None):
    return dict_to_element('game', d, source)
...
def root_to_pathsnames(games_root):
    diff_paths = []
    diff_names = []
...
def merge_gamelists(base_root, add_root, duplicate='i', source=None, updateonly=None):
    ...
    if duplicate == 'i':
        ...
        raise ValueError('Wrong argument value for duplicate in function merge_gamelists(): ' + duplicate)
    indent(base_root)
    indent(diff_root)
    return diff_root
...
def get_game_bypath(xml_root, path):
